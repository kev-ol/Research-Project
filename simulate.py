...
def simulate_data(Y_real, W_real, Z1_real, Z2_real, results_gibbs,
    ...
    Exogenous series are re-simulated as AR(1) processes fit to the real series.
    beta_0 and lambda are taken from the Gibbs posterior means; beta_c is drawn
    from the implied Minnesota prior. Sigma_c reuses the real posterior's average
    correlation structure with resampled variances. gamma_c is drawn from the
    Gibbs posterior mean and std.

    Parameters
    ----------
    Y_real, W_real, Z1_real, Z2_real : numpy.ndarray
        Real endogenous and exogenous panel data.
    results_gibbs : dict
        Real-data Gibbs output; must contain 'beta_0', 'lam', 'Sigma_c', 'gamma_c'.
    C, T : int
        Number of countries and usable time periods to simulate.
    burn : int, optional
        Initial periods simulated then discarded. Default is 50.
    ...
    -------
    true_params : dict
        Keys: 'beta_c', 'gamma_c', 'Sigma_c', 'beta_0', 'lam', 'Lambda'.
    Y : numpy.ndarray of shape (C, T, N)
        Simulated endogenous panel.
    W_sim, Z1_sim, Z2_sim : numpy.ndarray
        Simulated exogenous series.
    """
