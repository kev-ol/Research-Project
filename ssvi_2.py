...
def calc_V_beta0(lam, V_deltac, Lambda_inv, Lambda_inv_sum, C, N, K):
...
def calc_mu_beta0(lam, V_deltac, mu_sigma_inv, V_beta0, Y, F, Lambda_inv, Pc, C, N, K):
    lam = np.atleast_1d(lam)
    n = len(lam)
...
def calc_D(lam, mu_sigma_inv, Y, F, FF, Lambda_inv, Lambda_inv_sum, size_deltac, Pc, C, N, K):
    V_deltac = calc_V_deltac(lam, mu_sigma_inv, FF, Lambda_inv, size_deltac, Pc, C, N, K)

    V_beta0 = calc_V_beta0(lam, V_deltac, Lambda_inv, Lambda_inv_sum, C, N, K)
    mu_beta0 = calc_mu_beta0(lam, V_deltac, mu_sigma_inv, V_beta0, Y, F, Lambda_inv, Pc, C, N, K)
...
def calc_q_lambda(n_steps, step_size, lam_init, mu_sigma_inv, Y, F, FF, Lambda_inv, Lambda_inv_sum, size_deltac, Pc, C, N, K):
    log_lams = np.zeros(n_steps)
    Ds = np.zeros((n_steps, C))
    l = np.log(lam_init)
    for n in range(n_steps):
        lam = np.exp(l)
        D = calc_D(lam, mu_sigma_inv, Y, F, FF, Lambda_inv, Lambda_inv_sum, size_deltac, Pc, C, N, K)
        Ds[n] = D
        log_lams[n] = l
...
def calc_exp_lambda(lams, mu_sigma_inv, Ds, Y, F, FF, Lambda_inv, Lambda_inv_sum, size_deltac, Pc, C, N, K):
    lams = np.atleast_1d(lams)
    inv_lams = 1/lams
    V_deltac = calc_V_deltac(lams, mu_sigma_inv, FF, Lambda_inv, size_deltac, Pc, C, N, K)

    V_beta0 = calc_V_beta0(lams, V_deltac, Lambda_inv, Lambda_inv_sum, C, N, K)
    mu_beta0 = calc_mu_beta0(lams, V_deltac, mu_sigma_inv, V_beta0, Y, F, Lambda_inv, Pc, C, N, K)
    ...
    # core is now batched: (n, N*K, N*K), since V_beta0 varies per lambda draw
    core = [np.einsum('ij,njk,kl->nil', Lambda_inv[c], V_beta0, Lambda_inv[c]) for c in range(C)]
...
def calc_ELBO(exp_logdet_V_beta0, exp_logdet_V_deltac, S_bar_sigma, mu_log_lambda, mu_lambda_inv_D, mu_log_q_lambda, C, N, K, T):
...
def run_ssvi(ssvi_pack, Z_width, C, N, K, T, n_steps=1000, step_size = 0.01, n_burnin = 100):
    ...
    while len(ELBO) < 10 or ELBO[-1] - ELBO[-2] > epsilon:
        q_lambda, Ds = calc_q_lambda(n_steps, step_size, lam_init, mu_sigma_inv, Y, F, FF, Lambda_inv, Lambda_inv_sum, size_deltac, Pc, C, N, K)
        q_lambda = q_lambda[n_burnin:]
        Ds = Ds[n_burnin:]
        lam_init = q_lambda[-1]
        exp_mu_deltac, cov_deltac, mu_log_lambda, mu_log_q_lambda, exp_logdet_V_beta0, exp_logdet_V_deltac, mu_lambda_inv_D = calc_exp_lambda(
            lams, mu_sigma_inv, Ds, Y, F, FF, Lambda_inv, Lambda_inv_sum, size_deltac, Pc, C, N, K)
        #if len(ELBO)>0:
        ...
        elbo_after_sigma = calc_ELBO(exp_logdet_V_beta0, exp_logdet_V_deltac, S_bar_sigma, mu_log_lambda, mu_lambda_inv_D, mu_log_q_lambda, C, N, K, T)
        #if len(ELBO)>0:
